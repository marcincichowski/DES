...
class Window:
    def __init__(self, master):
        ...
        data_label = Label(master, text="Informacja").grid(row=0, column=0)
        key_label = Label(master, text="Klucz szyfrujący").grid(row=1, column=0)
    ...
    def callback_encrypt(self):
        if not self.key.get() or not self.data.get():
            ...
            try:
                ...
                print('error inputs')
                return messagebox.showinfo('Błąd!', 'Wprowadź dane heksadecymalne!')
